...
void InputTester::Update()
{
	...
	if (Input::GetKeyDown(GLFW_KEY_F)) GameTime::ToggleFPS(); // ** Do not use for sampling -- use below method instead
	if (Input::GetKeyDown(GLFW_KEY_R)) {
		int modSampleSize = 1; // **number of samples per FPS sample
		GameTime::SetSampleSetSize(modSampleSize); // **set the number of samples per FPS sample. 
		GameTime::ModSampleSize(SAMPLE_SIZE_TEST = 25); // **sets sample size to 25 (how many measurements to be done). sample size is then modified to size necessary to take averaged samples.
	}
	...
	gameObject->transform->Translate(glm::vec3(Input::GetDeltaPosX() * GameTime::deltaTime, 0.0f, Input::GetDeltaPosY() * GameTime::deltaTime));
...
void InputTester::fpsSample(int &sample_sz)
{
	...
	{
		--sample_sz;
		GameTime::GetFPS(logFPS); // **passes logFPS function as callback function.
	}
}
...
void InputTester::PrintFPS()
{
	if (GameTime::getInstance().IsCounting()) // **use IsCounting function to find out if FPS system is running.
		GameTime::GetFPS(printFPS); // **passes printFPS function as callback function.
}